...
def _significant_voids(data):
    return [c for c in data.dfn.components.wet
            if c.family == 'void' and c.size >= 5]
...
def test_at_probe_inherits_query_options():
    ...
    network = DelaunayFlowNetwork.from_arrays(coords, radii, epsilon=1e-7)
    result = network.get_topography(probe_radius=1.4, min_size=0, transit_policy='resident_only')
