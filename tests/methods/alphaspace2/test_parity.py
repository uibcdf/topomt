...
def _native_pocket_scores_from_state(state) -> np.ndarray:
    ...
    return np.array(
        [
            float(np.sum(beta_scalar_scores[beta_indices]))
...
def test_alphaspace2_native_state_matches_upstream_snapshot(pdb_name):
    ...
    upstream_pocket_lining_sets = {
        tuple(
            sorted(
                np.unique(
                    np.asarray(pocket.lining_atoms_idx, dtype=int).reshape(-1)
                ).tolist()
            )
        )
    ...
    native_pocket_lining_sets = {
        tuple(
            sorted(
                np.unique(state.alpha_lining_local_indices[alpha_indices].reshape(-1))
                .astype(int)
                .tolist()
            )
        )
    ...
    upstream_pocket_scores = np.array(
        [float(pocket.score) for pocket in snapshot.pockets], dtype=float
    )
...
def test_alphaspace2_native_grid_volume_matches_helper():
    ...
    expected = np.array(
        [
            _grid_volume(state.alpha_centers_nm[indices])
            for indices in state.pocket_alpha_index_list
        ],
        dtype=float,
    )
...
def test_alphaspace2_native_overlap_matrices_match_helper():
    ...
    intersection, union = _overlap_matrices(
        state.pocket_alpha_index_list, len(state.alpha_centers_nm)
    )
...
def test_alphaspace2_beta_overlap_matches_helper():
    ...
    intersection, union = _overlap_matrices(
        state.beta_alpha_index_list, len(state.alpha_centers_nm)
    )
...
def test_alphaspace2_native_state_matches_upstream_snapshot_for_cdk2_vina_scores():
    ...
    alphaspace2.annotateVinaAtomTypes(
        receptor=receptor, pdbqt=str(UPSTREAM_CDK2_PDBQT_PATH)
    )
    ...
    assert len(state.beta_centers_nm) == len(snapshot._beta_xyz)
    assert (
        np.asarray(state.beta_scores).shape == np.asarray(snapshot._beta_scores).shape
    )
    assert np.allclose(
    ...
    native_pocket_scores = _native_pocket_scores_from_state(state)
    assert np.allclose(
        upstream_pocket_scores, native_pocket_scores, atol=5.0e-4, rtol=0.0
    )
...
def test_alphaspace2_native_state_matches_upstream_contact_flags():
    ...
    assert np.array_equal(
        np.asarray(snapshot._alpha_contact, dtype=bool),
        np.asarray(contacts, dtype=bool),
    )
    assert np.array_equal(
        np.asarray(snapshot._alpha_contact, dtype=bool),
        np.asarray(state.alpha_contact, dtype=bool),
    )
    assert np.array_equal(
        np.asarray(snapshot._beta_contact, dtype=bool),
        np.asarray(state.beta_contact, dtype=bool),
    )
    assert np.array_equal(
        np.asarray(snapshot._pocket_contact, dtype=bool),
        np.asarray(state.pocket_contact, dtype=bool),
    )
