# Alpha-Flow Network Decomposition (AFND): Pertinence and Integration Analysis within TopoMT

This document analyzes the strategic fit of the AFND module within the existing architecture and philosophy of the TopoMT library. It validates that AFND is not merely an add-on, but a core engine that aligns with and empowers the library's hierarchical design.

## 1. Philosophical Alignment

**TopoMT Goal:** "Hierarchical analysis of molecular surface topography."
**AFND Contribution:** AFND provides the mathematical mechanism to generate this hierarchy. By decomposing the space into a flow network, it naturally distinguishes and relates topological features (Pockets, Channels, Voids) based on physical connectivity rules, fulfilling the "hierarchical" promise of the library's name.

**Verdict:** AFND is the native algorithmic realization of TopoMT's core philosophy.

## 2. Architectural Integration

### 2.1. The "Engine" Pattern
The relationship between AFND and the existing codebase should follow a **Generator/Product** pattern.

*   **The Generator (AFND):** The `AlphaFlowNetwork` class acts as the computational engine. It ingests atomic coordinates and builds the persistent, parameter-independent dual graph (Delaunay + Permeability Map).
*   **The Configuration:** The user supplies specific query parameters ($R_{probe}$, $R_{sea\_level}$).
*   **The Product (`Topography`):** The result of querying the AFND engine is a populated `Topography` object containing instances of `Pocket`, `Channel`, `Mouth`, etc.

**Implication:**
AFND does not replace `topomt.topography.Topography`; it populates it. The `Topography` class becomes the container for the specific "instance" of the topography at a given resolution, derived from the underlying AFND model.

### 2.2. Mapping to Existing Feature Classes
TopoMT already defines a rich ontology in `topomt/features/`. AFND provides a direct, 1-to-1 mapping for these classes:

| AFND Concept | TopoMT Class | Mapping Logic |
| :--- | :--- | :--- |
...
**Verdict:** High architectural compatibility. AFND will act as a factory that instantiates these existing classes, giving them rigorous geometric definitions.

## 3. Data Flow and Dependencies

### 3.1. Input: MolSysMT
AFND will strictly adhere to the `MolSysMT` ecosystem for input handling.
...
afn = AlphaFlowNetwork(molecular_system)
