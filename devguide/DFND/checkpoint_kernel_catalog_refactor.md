...
- **Phase 4** (touches the guard + docs): reframe `output_status.py` to track the
  **catalog** outputs (classify names) instead of kernel families -- the guard must
  ...
  cell) -- **NOT additive**: touches `families.py`, `SIDE_BY_FAMILY`, the guard,
  tests, done together with the output_status reframe. Rename septum->constriction
  in docs (the code already uses `constriction` for the wet<->wet wall). Reconcile
  the broad doc set (feature_definitions §5/§5.2.1, Glossary, Algorithm, Overview,
  interfaces, known_limitations L1.1/L3.1).
