...
| `1 mouth, resident, occlusion ≤ 1` | **`open_concavity`** *(open)* | **`cleft`** (deep, by buriedness) 🔶 / **`groove`** (elongated, by elongation) 🔶 / `dish` ⏳ (descriptors) | ✅ generic / 🔶 `cleft` (depth) + `groove` (elongation) provisional thresholds (S12); `funnel` is a *motif* (access), not a leaf |
