...
- Source version: `gh-run-receptor@1.1.0`
...
Version `1.0.0` is the stable read-only contract with:
...
are not implemented in `1.0.0`. Private-repository and fork token behavior remains
...
without `$/` are not claimed. Pin the full 1.1.0 commit instead of the tag when an
...
Version 1.0.0 consumes attempt-qualified `events@1` artifacts emitted by a producer and
...
Version `1.0.0` supports exactly one identity per rule: an exact `path`, positive numeric
