...
class Feature2D(BaseFeature):
    ...
    def __copy__(self):
        ...
        new_feature.solvent_accessible_volume = copy.copy(
    ...
    def __deepcopy__(self, memo):
        ...
        new_feature.solvent_accessible_volume = copy.deepcopy(
            self.solvent_accessible_volume, memo
        )
        new_feature.molecular_surface_area = copy.deepcopy(
            self.molecular_surface_area, memo
        )
        new_feature.molecular_surface_volume = copy.deepcopy(
            self.molecular_surface_volume, memo
        )
    ...
    def add_connected_boundary(self, feature_or_id: 'BaseFeature | str'):
        ...
        if self._topography is None:
            raise ValueError(
                'Topography is not set for this feature. Cannot add connected boundary.'
            )
