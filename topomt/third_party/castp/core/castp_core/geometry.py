...
def _edge_rho_map(
    ...
    for edge_atom_indices, opposite_vertices in edge_to_opposite_vertices.items():
        ...
        result[edge_atom_indices] = 0.0 if attached else float(edge_power_value)
    ...
    for simplex_index, simplex_neighbors in enumerate(mesh.neighbors):
        ...
        for face_index, neighbor in enumerate(simplex_neighbors):
            ...
            is_attached = (
                ...
                ) == 1  # degenerate (return=2) is treated as NOT attached, per C hidden_triangle
            )
...
def build_castp_geometry(
    ...
    edge_rho_values_array = np.asarray(list(edge_rho_map.values()), dtype=float)
    ...
    rho_spectrum_source = np.concatenate(
        (
            ...
            edge_rho_values_array[np.abs(edge_rho_values_array) > 1e-12],
    ...
    edge_rho_ranks: dict[tuple[int, int], int] = {
        edge: (_rank_of_value(spectrum_values, rho) if abs(rho) > 1e-12 else 0)
        for edge, rho in edge_rho_map.items()
    }
