...
def _mouth_face_initial_edge_facets(
    ...
    rank2: int | None,
    pocket_simplex_indices: set[int] | None = None,
...
def _mouth_face_outward_atoms(
    ...
    if pocket_simplex_indices is not None:
        sink_is_outside = int(simplex_index) not in pocket_simplex_indices
    else:
...
def _fnext_walk_around_edge(
    ...
    visited_states: set[tuple[int, tuple[int, int, int]]] = set()
    ...
    for _ in range(max_steps):
        ...
        if pocket_simplex_indices is not None:
            if int(next_tet) not in pocket_simplex_indices:
                return next_edge_facet
        elif depth[next_tet] == infinity_marker:
...
def _cluster_mouth_faces_fnext(
    ...
    simplex_rho_ranks: np.ndarray | None,
    rank2: int | None,
    pocket_simplex_indices: set[int] | None,
    ...
    parents = list(range(len(face_records)))
