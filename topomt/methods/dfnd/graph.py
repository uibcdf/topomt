...
class DelaunayFlowNetwork:
    ...
    def __init__(self, molecular_system, selection='all', structure_indices=0, epsilon=1e-6):
        """
        Initializes the DelaunayFlowNetwork by building the Delaunay mesh and pre-calculating
    ...
    def get_topography(self, probe_radius=1.4, sea_level=10.0, min_size=4):
        """
        Queries the pre-built DelaunayFlowNetwork for a specific probe radius
        ...
        adj_matrix_wet = coo_matrix(
            (valid_weights_wet, (valid_sources_wet, valid_targets_wet)),
            shape=(self.n_tetrahedra, self.n_tetrahedra),
        )

        n_comps_wet, labels_wet = connected_components(adj_matrix_wet, directed=False)

        # Identify mouths by checking which transit components connect to ocean.
        ...
        transit_nodes_at_mouth = np.where(
            mask_transit[mouth_sources], mouth_sources, mouth_targets
        )
        ...
        # A node is COAST if:
        # 1. It is NOT Transit and NOT Ocean.
        ...
        # Wet = Transit + Coast, so Dry = Solid - Coast.
        mask_dry_final = mask_candidate_coast.copy()
        ...
        adj_matrix_dry = coo_matrix(
            (valid_weights_dry, (valid_sources_dry, valid_targets_dry)),
            shape=(self.n_tetrahedra, self.n_tetrahedra),
        )
