...
def hollow_cube(
    half=8.0, wall_spacing=3.5, atom_radius=ARGON_VDW_RADIUS, jitter=0.0, seed=0
):
...
def hollow_sphere_patchy(
    ...
    coords, _r = hollow_sphere(
        sphere_radius, wall_spacing, atom_radius, jitter=0.0, seed=seed
    )
    rng = np.random.default_rng(seed)
...
def hollow_sphere_with_opening(
    sphere_radius=10.0,
    wall_spacing=3.5,
...
def solid_ball(
    ball_radius=8.0, spacing=3.5, atom_radius=ARGON_VDW_RADIUS, jitter=0.0, seed=0
):
...
def nested_spheres(
    ...
    outer, _o = hollow_sphere(
        outer_radius, wall_spacing, atom_radius, jitter=0.0, seed=seed
    )
    inner, _i = hollow_sphere(
        inner_radius, wall_spacing, atom_radius, jitter=0.0, seed=seed + 1
    )
...
def curved_tube(
    ...
    axis_pts = np.stack(
        [
            bend_radius * np.cos(angles),
            bend_radius * np.sin(angles),
...
def flask(
    ...
    chamber, _c = hollow_sphere(
        chamber_radius, wall_spacing, atom_radius, jitter=0.0, seed=seed
    )
    cos_cut = np.cos(np.deg2rad(opening_half_angle_deg))
...
def hollow_sphere_two_openings(
    sphere_radius=10.0,
    wall_spacing=3.5,
    opening1_half_angle_deg=35.0,
    opening2_half_angle_deg=15.0,
    atom_radius=ARGON_VDW_RADIUS,
    jitter=0.1,
    seed=0,
):
...
def asymmetric_dumbbell(
    ...
    s1, _a = hollow_sphere(
        lobe_radius1, wall_spacing, atom_radius, jitter=0.0, seed=seed
    )
    s2, _b = hollow_sphere(
        lobe_radius2, wall_spacing, atom_radius, jitter=0.0, seed=seed + 1
    )
...
def void_with_island(
    ...
    shell, _s = hollow_sphere(
        sphere_radius, wall_spacing, atom_radius, jitter=0.0, seed=seed
    )
    island, _i = solid_ball(
        island_radius, island_spacing, atom_radius, jitter=0.0, seed=seed
    )
...
def helical_tube(
    ...
    axis_pts = np.stack(
        [helix_radius * np.cos(t), helix_radius * np.sin(t), pitch * t / (2.0 * np.pi)],
        axis=1,
    )
    segments = [
        _tube_surface(axis_pts[i], axis_pts[i + 1], tube_radius, wall_spacing)
...
def onion_shells(
    ...
    for k, sphere_radius in enumerate(radii):
        pts, _r = hollow_sphere(
            sphere_radius, wall_spacing, atom_radius, jitter=0.0, seed=seed + k
        )
        shells.append(pts)
...
def pocket_with_mouth_intruder(
    intruder=True,
    sphere_radius=9.0,
    wall_spacing=3.5,
    ...
    if intruder:
        mouth = np.array(
            [[0.0, sphere_radius * cos_cut, 0.0]]
        )  # one atom in the opening
...
def rough_surface(
    ...
    rng = np.random.default_rng(seed)
    layers = [
        np.stack([flat_x, flat_y, np.full(flat_x.size, -layer * spacing)], axis=1)
        for layer in range(1, n_layers)
    ]  # filled backing below z=0
    bump_z = amplitude * np.sin(flat_x) * np.cos(flat_y) + rng.normal(
        scale=0.35, size=flat_x.size
    )
...
def two_balls(
    ...
    coords = np.vstack(
        [ball + np.array([-offset, 0.0, 0.0]), ball + np.array([offset, 0.0, 0.0])]
    )
    return _finalize(coords, atom_radius, jitter, seed)
...
def two_blocks(
    gap=4.0, half=7.0, spacing=3.2, atom_radius=ARGON_VDW_RADIUS, jitter=0.1, seed=0
):
...
def two_blocks_interface_slabs(
    gap=5.0,
    half=7.0,
    spacing=3.2,
    n_layers=2,
    ...
    """Two facing slabs cut from the interface side of two cubic blocks.

    This is the local-interface counterpart of ``two_blocks``: only the
    ``n_layers`` x-layers closest to the gap are kept for each block, while the
    original jitter sequence is preserved before cropping so the default case
    reproduces the hand-cropped interface fragment.
    """
    block = solid_block(half, spacing)
    offset = half + gap / 2.0
    left = block + np.array([-offset, 0.0, 0.0])
    ...
    left_inner = float(left[:, 0].max())
    right_inner = float(right[:, 0].min())
    layer_width = (max(1, int(n_layers)) - 1) * spacing + 1e-9
    mask = ((coords[:, 0] >= left_inner - layer_width) & (coords[:, 0] < 0.0)) | (
        (coords[:, 0] <= right_inner + layer_width) & (coords[:, 0] > 0.0)
    )
...
def three_blocks(
    gap=4.0, half=6.0, spacing=3.2, atom_radius=ARGON_VDW_RADIUS, jitter=0.1, seed=0
):
...
def interface_pocket(
    ...
    coords = np.vstack([left, right])
    carved = (
        np.linalg.norm(coords, axis=1) < pocket_radius
    )  # carve cavity at contact centre
...
def three_body_junction(
    ...
    for k in range(3):
        ...
        center = np.array(
            [place_radius * np.cos(angle), place_radius * np.sin(angle), 0.0]
        )
...
def packed_blob(
    ...
    while len(accepted) < target and attempts < 60 * target:
        ...
        if (
            accepted
            and np.min(np.linalg.norm(np.asarray(accepted) - p, axis=1))
            < min_separation
        ):
            continue
        accepted.append(p)
...
def pocket_in_pocket(
    ...
    well = (radial < inner_radius) & (
        grid[:, 2] > thickness - outer_depth - inner_depth
    )
...
def u_channel(
    ...
    left_arm = (
        (np.hypot(x + arm_separation / 2.0, y) < tube_radius)
        & (z > depth - depth)
        & (z < depth)
    )
    right_arm = (np.hypot(x - arm_separation / 2.0, y) < tube_radius) & (z < depth)
    bottom = (np.hypot(z - tube_radius, y) < tube_radius) & (
        np.abs(x) < arm_separation / 2.0
    )
...
def edge_cavity(
    ...
    carved |= (
        np.linalg.norm(grid - (corner - np.array([depth, depth, depth]) / 2.0), axis=1)
        < bowl_radius * 0.6
    )
    return _finalize(grid[~carved], atom_radius, jitter, seed)
...
def to_pdb(coords, radii, path, element='AR', resname='DUM', elements=None):
    ...
    for i, (x, y, z) in enumerate(coords, start=1):
        ...
        insertion = ''
        lines.append(
            f'{record:<6}{serial:5d} {elem:<4}{alt_loc:1s}{resname:>3s} '
            f'{chain:1s}{resseq:4d}{insertion:1s}   '
