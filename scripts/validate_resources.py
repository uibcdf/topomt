...
try:
    ...
    print(
        'ERROR: PyYAML is required. Install with: pip install pyyaml OR conda install -c conda-forge pyyaml'
    )
    raise
...
def validate_repo_field(repo: Any, file: Path, errors: List[ValidationError], ctx: str):
    if not is_str(repo) or not REPO_RE.match(repo):
        errors.append(
            ValidationError(
                file, f"{ctx}: 'repo' must be 'org/repo' string (got {repo!r})"
            )
        )
...
def validate_path_field(
    path_value: Any, file: Path, errors: List[ValidationError], ctx: str
):
    if not is_str(path_value) or path_value.strip() == '':
        errors.append(
            ValidationError(file, f"{ctx}: 'path' must be a non-empty string")
        )
        return
    if '://' in path_value:
        errors.append(
            ValidationError(
                file, f"{ctx}: 'path' should be a repo-relative path, not a URL"
            )
        )


def validate_unique_ids(
    items: List[Dict[str, Any]], file: Path, errors: List[ValidationError], kind: str
):
    seen = set()
...
def validate_talks(data: Any, file: Path, errors: List[ValidationError]):
    ...
    if not is_dict(data):
        errors.append(
            ValidationError(file, f'{ctx0}: top-level YAML must be a mapping')
        )
        return
    ...
    for i, t in enumerate(talks):
        ...
        if not is_list(authors) or not all(is_str(a) for a in authors):
            errors.append(
                ValidationError(file, f"{ctx}: 'authors' must be a list of strings")
            )
        ...
        if not is_dict(event):
            ...
            require_keys(
                event,
                ['name', 'location', 'date', 'duration_min'],
                f'{ctx}.event',
                errors,
                file,
            )
            if 'name' in event and not is_str(event['name']):
                errors.append(
                    ValidationError(file, f"{ctx}.event: 'name' must be a string")
                )
            if 'location' in event and not is_str(event['location']):
                errors.append(
                    ValidationError(file, f"{ctx}.event: 'location' must be a string")
                )
            if 'date' in event:
                d = event['date']
                if not is_str(d) or not ISO_DATE_RE.match(d):
                    errors.append(
                        ValidationError(
                            file, f"{ctx}.event: 'date' must be YYYY-MM-DD (got {d!r})"
                        )
                    )
            if 'duration_min' in event and not is_int(event['duration_min']):
                errors.append(
                    ValidationError(
                        file, f"{ctx}.event: 'duration_min' must be an integer"
                    )
                )
        ...
        if not is_str(status) or status not in TALK_STATUS:
            errors.append(
                ValidationError(
                    file,
                    f"{ctx}: 'status' must be one of {sorted(TALK_STATUS)} (got {status!r})",
                )
            )
        ...
        if artifacts is not None:
            if not is_dict(artifacts):
                errors.append(
                    ValidationError(
                        file, f"{ctx}: 'artifacts' must be a mapping if present"
                    )
                )
            else:
                for k, v in artifacts.items():
                    if not is_str(k) or not is_str(v):
                        errors.append(
                            ValidationError(
                                file,
                                f'{ctx}.artifacts: keys and values must be strings',
                            )
                        )

        if 'notes' in t and t['notes'] is not None and not is_str(t['notes']):
            errors.append(
                ValidationError(file, f"{ctx}: 'notes' must be a string if present")
            )
    ...
    for i, p in enumerate(papers):
        ...
        if not is_str(status) or status not in PAPER_STATUS:
            errors.append(
                ValidationError(
                    file,
                    f"{ctx}: 'status' must be one of {sorted(PAPER_STATUS)} (got {status!r})",
                )
            )
        ...
        if not is_list(related) or not all(
            is_str(r) and REPO_RE.match(r) for r in related
        ):
            errors.append(
                ValidationError(
                    file, f"{ctx}: 'related_repos' must be a list of 'org/repo' strings"
                )
            )
        ...
        if 'doi' in p and p['doi'] is not None and not is_str(p['doi']):
            errors.append(
                ValidationError(file, f"{ctx}: 'doi' must be a string if present")
            )
        if 'url' in p and p['url'] is not None and not is_str(p['url']):
            errors.append(
                ValidationError(file, f"{ctx}: 'url' must be a string if present")
            )
    ...
    for i, t in enumerate(tutorials):
        ...
        if not is_str(level) or level not in TUTORIAL_LEVEL:
            errors.append(
                ValidationError(
                    file,
                    f"{ctx}: 'level' must be one of {sorted(TUTORIAL_LEVEL)} (got {level!r})",
                )
            )
        ...
        if not is_str(fmt) or fmt not in TUTORIAL_FORMAT:
            errors.append(
                ValidationError(
                    file,
                    f"{ctx}: 'format' must be one of {sorted(TUTORIAL_FORMAT)} (got {fmt!r})",
                )
            )

        if 'est_time_min' in t and not is_int(t['est_time_min']):
            errors.append(
                ValidationError(file, f"{ctx}: 'est_time_min' must be an integer")
            )
        ...
        if not is_str(status) or status not in TUTORIAL_STATUS:
            errors.append(
                ValidationError(
                    file,
                    f"{ctx}: 'status' must be one of {sorted(TUTORIAL_STATUS)} (got {status!r})",
                )
            )
        ...
        if 'prerequisites' in t and t['prerequisites'] is not None:
            ...
            if not is_list(prereq) or not all(is_str(x) for x in prereq):
                errors.append(
                    ValidationError(
                        file,
                        f"{ctx}: 'prerequisites' must be a list of strings if present",
                    )
                )
...
def validate_file(path: Path) -> Tuple[bool, List[ValidationError]]:
    ...
    if path.name == 'talks.yml':
        ...
        errors.append(
            ValidationError(
                path,
                'Unknown resources file (expected talks.yml, papers.yml, tutorials.yml)',
            )
        )
...
def main() -> int:
    ...
    ap.add_argument(
        '--strict', action='store_true', help='Fail if any resources file is missing'
    )
    args = ap.parse_args()
    ...
    targets = [
        resources_dir / 'talks.yml',
        resources_dir / 'papers.yml',
        resources_dir / 'tutorials.yml',
    ]
